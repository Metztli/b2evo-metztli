...
class videoplug_plugin extends Plugin
{
	...
	function PluginInit( & $params )
	{
		...
		$this->long_desc = T_('This plugin allows to quickly embed (plug) a video from a video hosting site such as RuTube, YouTube, Vimeo, DailyMotion and Facebook. Use it through the toolbar or directly by entering a shortcode like [video:rutube:123xyz], [video:youtube:123xyz] or [video:vimeo:123xyz] into your post, where 123xyz is the ID of the video.');
	}
	...
	function RenderItemAsHtml( & $params )
	{
		...
		$content = move_short_tags( $content, '/\[video:(rutube|youtube|dailymotion|vimeo|facebook):?[^\[\]]*\]/i' );

		// Replace video tags with html code:
		$content = replace_content_outcode( '#\[video:(rutube|youtube|dailymotion|vimeo|facebook|google|livevideo|ifilm):([^:]+?):?(\d+%?)?:?(\d+%?)?]#',
	 ...
	 * @param array Matches:
	 *              0 - Full video tag
	 *              1 - Video type: rutube, youtube, dailymotion, vimeo, facebook, google, livevideo, ifilm
	 *              2 - Video code/key
	 *              3 - Width
	 *              4 - Height
	...
	function parse_video_tag_callback( $m )
	{
		...
		{
			case 'rutube':
				$video_block = '<iframe id="rtplayer" type="text/html" src="//www.rutube.com/embed/'.$m[2].'" allowfullscreen="allowfullscreen" frameborder="0"></iframe>';
				break;
	...
	function DisplayCodeToolbar( $params = array() )
	{
		...
		echo '<input type="button" id="video_rutube" title="'.T_('Insert Rutube video').'" class="'.$this->get_template( 'toolbar_button_class' ).'" data-func="videotag|rutube|'.$params['js_prefix'].'" value="RuTube" />';
